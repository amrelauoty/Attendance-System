$(document).ready(function () {
  ...
  users.forEach(function (user) {
    if (
      ...
      let startOfWork = document.createElement("td");
      let early = document.createElement("td");
      let late = document.createElement("td");
      let excuse = document.createElement("td");
      let absent = document.createElement("td");
      let attendance = JSON.parse(localStorage.getItem("userAttendance"));
      attendance.forEach(function (myuser) {
        if (!myuser.attendance.length == 0) {
          if (myuser.username == user.username) {
            let attlength = myuser.attendance.length;
            startOfWork.innerText = myuser.attendance[attlength - 1].date;
            let earlycounter = 0,
              latecounter = 0,
              excusecounter = 0,
              absentcounter = 0;
            myuser.attendance.forEach((day) => {
              switch (day.case) {
                case "attend":
                  earlycounter++;
                  break;
            ...
            early.innerText = earlycounter;
            late.innerText = latecounter;
            excuse.innerText = excusecounter;
            absent.innerText = absentcounter;
          }
        }
      });
      ...
      tr.appendChild(startOfWork);
      tr.appendChild(early);
      tr.appendChild(late);
      tr.appendChild(excuse);
